...
1. **`docs/guides/nextjs-caching.md`** — two stale passages:
   ...
   - ~Line 31, the "Granularity" paragraph: `SubmissionRubricSection` no longer exists. The cached `app/` sections today are `RubricHeaderSection` and `GradeTargetCriterionSection` (rubric grading page) and `GradeTargetGradingSection` (grade-target page). Also `submissionId` → `targetId`, "one entry per submission … first visit to each submission" → per target.
