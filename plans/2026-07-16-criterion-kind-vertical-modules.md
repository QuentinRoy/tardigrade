...
### PR2 — Number — **Implemented (branch `claude/active-plan-next-step-pmacfc`; PR pending)**
...
- **Test-seam migration** matched PR1: the `markNumberCriterion` cases moved into `criteria/number/numberDomain.test.ts` (the now-empty `criterion.test.ts` was deleted); added `numberBounds.test.ts` (invariant) and `numberPersistence.integration.test.ts` (batched upsert + coordinator dispatch). The Number cases in `schemas.test.ts`, `parseRubrics.test.ts`, `rubrics.test.ts`, `resultsBuilder.test.ts`, and `rubricsExport.test.ts` were **left in place** — they stay green through the new seams; folded into the PR4 test revision.
