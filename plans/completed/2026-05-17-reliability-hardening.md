...
- Tier 2: 0 open, 0 in progress, 4 verified
...
- Sprint label: Reliability Sprint A — complete (all six Tier 0 items Verified with merge-blocking `ci-status`); Tier 1 — complete (all seven items Verified); Tier 2 — complete (all four items Verified)
- Remaining scope: None — all 17 risks Verified.
- Exit criteria: Tier 2 action/route/UX error contracts covered by tests (met)
...
| R-012 | Tier 2 | 40 | Server action contracts | Question/import/grading actions need explicit tests for actionable error messaging and controlled failure paths. The interactive-grading action `saveAssessment` is a bare `"use server"` passthrough that re-throws DB/pool errors to the client with no recovery path. | src/questions/actions.ts, src/import/*ImportAction.ts, src/import/actionUtils.ts, src/assessments/saveAssessment.ts | Verified | [#31](https://github.com/QuentinRoy/grading/issues/31) | Unassigned | Sprint C | Implemented per `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` PR 1. Introduced `ImportBlockedError` (`src/import/importErrors.ts`); `assessmentImportBlockedError` (`src/import/saveAssessments.ts`) and `questionImportBlockedError` (`src/import/saveQuestions.ts`) now throw it instead of a bare `Error`, keeping blocking-diagnostic messages a recognized domain type. `src/assessments/saveAssessment.ts` (the `"use server"` action) now wraps the `persistAssessment` call in try/catch: unexpected throws log under the `assessments` scope and return `{ success: false, error: assessmentErrors.unexpected }`; the catch deliberately stays in `saveAssessment.ts` rather than `assessmentMutations.ts` so the import path (which composes `saveAssessmentInDb` directly) keeps throwing for transaction rollback. `src/import/actionUtils.test.ts`: ZodError → prettified, `ImportBlockedError` → message preserved, generic `Error`/non-Error → generic message + single log call, no raw text leak. `src/assessments/saveAssessment.integration.test.ts`: forces an infra throw by destroying the test database's connection pool mid-transaction and asserts a shaped `{ success: false }` result, one log call, and no cache invalidation. Each GREEN change was confirmed by reverting it and observing its RED test fail, then restoring. local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm run test:unit` (30 files, 183 tests passing), `pnpm run test:integration` (92 passing). | None — keep evidence current if these modules move. |
| R-013 | Tier 2 | 24 | UI optimistic save behavior | Assessment session optimistic updates and rollback/pending accounting are not explicitly tested. | src/assessments/useAssessmentSession.ts | Verified | [#30](https://github.com/QuentinRoy/grading/issues/30) | Unassigned | Sprint C | `src/assessments/useAssessmentSession.stories.tsx`: Storybook hook-harness play functions (per `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` PR 2) covering `SuccessCommitsSavedValue` (optimistic value visible + pending=1 while in flight, then saved value committed + pending=0 on success), `FailureRollsBackOptimisticValue` (optimistic value reverts to saved, pending=0, `onError` called once with the error), `OutOfOrderCompletionSettlesEachIndexCorrectly` (two `assess` calls on different rubric indices, second resolved before the first, both indices settle correctly and pending counters never go negative), and `SuccessAfterFailureKeepsPendingAccountingCorrect` (a failed save followed by a successful retry on the same index keeps pending accounting correct). The `saveRubric` stub returns a deferred promise per call so the play function controls completion order explicitly rather than via fixed delays. Characterization tests only — no behavior change found in `useAssessmentSession.ts`; all four stories passed on first run. Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm test:storybook useAssessmentSession` (4 tests passing) | None — keep evidence current if these modules move. |
| R-014 | Tier 2 | 27 | Export routes API behavior | Route response contracts (404/400/200, headers, filenames) need route-level tests. The submissions route also called `createCsvSubmissionExport` with no catch, so a thrown export/DB error surfaced as an unshaped 500 with no log, unlike the questions route. | app/projects/[projectId]/[projectSlug]/export/questions/route.ts, app/projects/[projectId]/[projectSlug]/export/submissions/route.ts | Verified | [#28](https://github.com/QuentinRoy/grading/issues/28) | Unassigned | Sprint C | `app/projects/[projectId]/[projectSlug]/export/questions/route.test.ts` and `app/projects/[projectId]/[projectSlug]/export/submissions/route.test.ts` (new, co-located, in the `unit` Vitest project after extending its `include` glob to also match `app/**/*.{test,spec}.{ts,tsx}` in `vitest.config.ts`): both routes covered for project-not-found (404 `{error}`), happy path (200 with `content-type`, dated/slug-based `content-disposition` filename, and the submissions route's `cache-control: no-store`), and a thrown export/DB error (500 `{error}` + one `logger.error` call under the `export` scope, ADR 0009); the submissions route additionally covers invalid query options (400 `{error}` via the route-internal `parseExportOptions`, also covered by a repeated-include-params case asserting the parsed options reach `createCsvSubmissionExport`). Behavior change (TDD red/green, confirmed by reverting and observing RED, then restoring): the submissions route now wraps `createCsvSubmissionExport` in try/catch, shaping thrown errors into `500 {error}` JSON and logging via `createLogger("export")`, matching the questions route's existing shape; the questions route gained the same `logger.error` call in its existing catch block, since it previously shaped the JSON but never logged. Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm run test:unit` (27 files, 172 tests passing). | None — keep evidence current if these modules move. |
| R-015 | Tier 2 | 20 | Operational signal quality | Some paths surface generic errors; consistency of user-recoverable guidance should be tested and standardized. | app/projects/page.tsx, src/import/actionUtils.ts | Verified | [#29](https://github.com/QuentinRoy/grading/issues/29) | Unassigned | Sprint C | Implemented per `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` PR 1, alongside R-012 (same error-classification rule). `app/projects/page.tsx`'s error-shaping logic moved into a new co-located `src/projects/createProjectErrorMessage.ts` (feature-owned per ADR 0002, not under `app/`, since it is domain logic rather than route composition): `toCreateProjectErrorMessage` keeps the specific "Project name is required." message and otherwise logs under the `projects` scope and returns a generic `projectMessages.unexpected` message — never the raw `error.message`. `src/questions/errors.ts`'s `toQuestionsValidationError` keeps `QuestionsValidationError`/`ZodError` branches and now logs under `questions` + returns a generic `questionsMessages.unexpected` message for any other error. Audited the bare-`Error` trap at `questionDefinitionMutations.ts:85` ("Question id is required.", defensive/unreachable given `idSchema`'s zod `min(1)`) and routed it through `QuestionsValidationError` so it stays a recognized domain message. `src/import/actionUtils.ts`'s `toImportErrorState` gained the same generic-fallback + single-log-call shape via the new `ImportBlockedError` type (see R-012). `src/projects/createProjectErrorMessage.test.ts` and `src/questions/errors.test.ts` assert the specific message is kept, the generic message never leaks internal text, and the scoped logger is called exactly once for unexpected errors. local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm run test:unit`. | None — keep evidence current if these modules move. |
...
- [x] M6: Tier 2 tests implemented
- [x] M7: Tier 2 verified in CI
...
- 2026-06-23: Implemented PR 2 of `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` (Optimistic-save hook tests). Added `src/assessments/useAssessmentSession.stories.tsx`, a Storybook hook-harness following `useSubmissionQuickJump.stories.tsx`'s precedent: a `Harness` component wires `useAssessmentSession` to a mixed boolean/ordinal/numerical `initialRubrics` fixture, a `saveRubric` stub that returns a per-call deferred promise (so play functions control completion order and timing explicitly, never fixed delays), and an `onError` spy, rendering `pendingByIndex`/`savedRubrics`/`optimisticRubrics` as queryable text. Four play functions cover the acceptance criteria: success (optimistic value visible + pending=1 while in flight, then committed + pending=0), failure rollback (optimistic value reverts, pending=0, `onError` called once), out-of-order completion across two indices (second call resolved before the first; both settle correctly, pending counters never go negative), and success-after-failure on the same index (pending accounting stays correct through a failed-then-successful retry). These are characterization tests only — no behavior change was needed in `useAssessmentSession.ts`; all four stories passed on the first run. Promoted R-013 to Verified. Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm test:storybook useAssessmentSession` (4 tests passing). Dashboard updated (Tier 2: 1 verified, 3 open; total 17; 14/17 verified, 3 remaining).
- 2026-06-23: Implemented PR 3 of `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` (export route tests + submissions-route error shaping). Extended the unit Vitest project's `include` glob in `vitest.config.ts` to also match `app/**/*.{test,spec}.{ts,tsx}` (it previously only matched `src/**`, so the two export routes were never discovered). Added co-located `route.test.ts` files for both export routes covering project-not-found (404), happy path (200 with headers/filename), a thrown export/DB error (500 + one log call), and, for the submissions route, invalid query options (400) plus repeated-include-params parsing. Behavior change via TDD (red/green, confirmed by reverting and observing RED, then restoring): the submissions route now wraps `createCsvSubmissionExport` in try/catch and shapes thrown errors into `500 {error}` JSON with a `createLogger("export")` log call, matching the questions route's existing JSON shaping; the questions route's existing catch block gained the same log call, since it shaped JSON but never logged. The submissions route's standalone `exportOptions.ts` query parser (and its separate test file, previously orphaned by the narrower glob) was folded back into `route.ts` as a route-internal function once route-level tests could cover its parsing contract directly. Promoted R-014 to Verified; dashboard updated (Tier 2: 2 open, 2 verified; total 17; 15/17 verified, 2 remaining). Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm test:unit` (27 files, 172 tests passing). Milestones M6/M7 are not marked complete here — R-012 and R-015, the remaining two Tier 2 risks in this plan, are landing via a separate PR (PR 1) not yet merged at the time this PR lands; whoever merges that PR should flip M6/M7 and move this plan to `plans/completed/`.
- 2026-06-23: Implemented PR 1 of `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` (consistent server-action error shaping), closing GitHub issues #31 (R-012) and #29 (R-015) together since they share one rule: in each feature's error function, a recognized domain/validation error keeps its specific message, and anything else is logged once under the feature's ADR-0009 scope and replaced with a generic, actionable message — never the raw `error.message`. Introduced `ImportBlockedError` (`src/import/importErrors.ts`) so the import path's blocking-diagnostic errors (`saveAssessments.ts`, `saveQuestions.ts`) stay a recognized domain type instead of a bare `Error` that a naive generic-fallback would have swallowed. Wrapped `src/assessments/saveAssessment.ts` (the interactive-grading `"use server"` action) in try/catch, added `assessmentErrors.unexpected`, and kept the catch out of `assessmentMutations.ts` so the import path's direct `saveAssessmentInDb` calls still throw for transaction rollback. Applied the same rule to `src/questions/errors.ts` (new `questionsMessages`) and a new co-located `src/projects/createProjectErrorMessage.ts` (feature-owned per ADR 0002 — moved out of `app/projects/` after review, since it is domain logic rather than route composition; `projectMessages` lives alongside it). Audited and closed the bare-`Error` domain-message trap at `questionDefinitionMutations.ts:85` ("Question id is required.") by routing it through `QuestionsValidationError`. New tests: `src/import/actionUtils.test.ts`, `src/questions/errors.test.ts`, `src/projects/createProjectErrorMessage.test.ts`, `src/assessments/saveAssessment.integration.test.ts` (forces an infra throw by destroying the test database's pool mid-transaction). Each behavior change followed red/green TDD, confirmed by reverting and observing the RED test fail, then restoring. Promoted R-012 and R-015 to Verified — all four Tier 2 risks (R-012, R-013, R-014, R-015) are now Verified; flipped milestones M6/M7 and moved `plans/completed/2026-06-23-tier2-action-route-ux-hardening.md` to `plans/completed/`. Dashboard updated (Tier 2: 0 open, 4 verified; total 17; 17/17 verified, 0 remaining). Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm run test:unit`, `pnpm run test:integration`.
...
- [x] Dashboard counts match risk table statuses.
- [x] Every Open/In Progress item has Owner, Target, and Next Action. (no Open/In Progress items remain)
- [x] Every Mitigated/Verified item includes Test Evidence.
- [x] Score reviewed for all items touched this week.
- [x] GitHub issue numbers in Issue # column match each risk.
- [x] PR body includes an auto-close keyword for each addressed risk issue (for example: `Fixes #<issue>`).
- [x] Any new risks have corresponding GitHub issues created and linked. (no new risks added this pass)
