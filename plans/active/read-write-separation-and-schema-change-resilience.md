...
Status: Proposed, needs refresh before execution
Date: 2026-05-18
...
This plan remains useful, but it should be treated as a proposed execution plan rather than a final architecture decision.

It owns the implementation strategy for separating write-side command/persistence paths from read-side projection/reporting paths. It does not own the broader source-structure audit, terminology decisions, product workflow decisions, or final caching/loading strategy.
...
- This plan owns the proposed read/write separation execution strategy.

Before implementation starts, this plan should be either:

- refreshed and kept in `plans/active/` if it is the next execution track;
- promoted to `docs/design/` if the read/write separation approach is accepted as the intended design;
- moved to `docs/investigations/` if it remains exploratory rather than executable.
...
Reference: [`plans/active/reliability-hardening.md`](reliability-hardening.md)
...
- This plan owns the proposed code-boundary extraction strategy.
...
- This plan owns the narrower execution strategy for separating write operations from read projections.
...
These overlap with the source-structure audit. The difference is that this plan specifically prioritizes separating writes from read/projection code so future schema changes affect fewer modules.
...
Risk: this plan conflicts with terminology or product investigations.
...
2. Reconcile this plan with #115, #117, #51, and the reliability tracker.
3. Extract a focused project route-context issue because it cuts across IDs, slugs, and project-scoped loading.
4. Start with write-side extraction in one central path, such as assessment save or question/rubric save.
5. Continue with read projection extraction for export and progress.
6. Update reliability tracker entries with concrete test evidence and reduced risk scores.
...
- #117, to show this plan has moved from proposed to execution.
...
- Re-score R-003, R-005, R-006, R-007, and R-011 after Phase C.
