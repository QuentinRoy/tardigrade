...
- **PR6 `questions: share cached question rows`** (Findings 5, 6). One private `loadQuestionRowsCached` scope; `loadQuestionRows`, `loadQuestionGrid`, and `loadQuestion` are plain public wrappers with no `db` parameter — they simply delegate to the shared cached source. The `...FromDb` primitive is the test seam; wrapper integration tests that used a `db` handle are removed (ADR 0007 rule 14). Amend ADR 0007 rules 13–14 and ADR 0008 rule 5 to prohibit `db` on cached wrappers and retire the dispatch recommendation. Add grandfathered comments to `loadSubmissions`, `loadQuestionAssessment`, and `loadSubmissionAssessments`. Lock Decision 5 (whole-project-set load tradeoff) at the call site. **Acceptance**: runtime reads needing the same project-wide question rows share one cache entry; no cached wrapper's type signature can express a `db` handle; ADRs updated; grandfathered wrappers commented; the `loadQuestion` breadth choice is documented. **Depends on**: PR2.
