...
Status: Active
Last Updated: 2026-06-23 (R-014 promoted to Verified — export route tests + submissions-route error shaping)
...
- Tier 2: 3 open, 0 in progress, 1 verified
...
- 2026-06-23: Implemented PR 3 of `plans/active/2026-06-23-tier2-action-route-ux-hardening.md` (export route tests + submissions-route error shaping). Extended the unit Vitest project's `include` glob in `vitest.config.ts` to also match `app/**/*.{test,spec}.{ts,tsx}` (it previously only matched `src/**`, so the two export routes — and an existing orphaned `exportOptions.test.ts` — were never discovered). Added co-located `route.test.ts` files for both export routes covering project-not-found (404), happy path (200 with headers/filename), a thrown export/DB error (500 + one log call), and, for the submissions route, invalid query options (400). Behavior change via TDD (red/green, confirmed by reverting and observing RED, then restoring): the submissions route now wraps `createCsvSubmissionExport` in try/catch and shapes thrown errors into `500 {error}` JSON with a `createLogger("export")` log call, matching the questions route's existing JSON shaping; the questions route's existing catch block gained the same log call, since it shaped JSON but never logged. Promoted R-014 to Verified; dashboard updated (Tier 2: 3 open, 1 verified; total 17; 14/17 verified, 3 remaining). Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm test:unit` (28 files, 173 tests passing). Milestones M6/M7 are not marked complete here — R-012, R-013, and R-015 (the other three Tier 2 risks in this plan) are landing via separate parallel PRs not confirmed in this session; whoever lands the last of the four should flip M6/M7 and move this plan to `plans/completed/`.
...
- [x] Every Open/In Progress item has Owner, Target, and Next Action.
...
- [ ] Any new risks have corresponding GitHub issues created and linked. (no new risks added this pass)
