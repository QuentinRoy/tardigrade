...
Status: Active
Last Updated: 2026-06-22 (second-pass repo audit: milestones M2/M3 closed; sprint focus + Section 6/7 refreshed; R-016/#22, R-006/#32, R-011/#25 issue-sync reconciled; new gaps flagged — see Change Log)
...
- Tier 1: 2 open, 0 in progress, 5 verified
...
- Sprint label: Reliability Sprint A — complete (all six Tier 0 items Verified with merge-blocking `ci-status`)
- Remaining scope: R-009, R-010 (Tier 1), then R-012, R-013, R-014, R-015 (Tier 2)
- Exit criteria: Tier 1 correctness contracts and Tier 2 action/route/UX error contracts covered by tests
...
| R-009 | Tier 1 | 54 | Import/export roundtrip | Export includes both assessment and marks columns; import behavior may diverge if only marks columns are present. | src/export/submissionExportCsv.ts, src/import/prepareAssessmentImport.ts, src/import/parseAssessments.ts | Open | [#27](https://github.com/QuentinRoy/grading/issues/27) | Unassigned | Sprint B | Groundwork (2026-06-10): `src/import/prepareAssessmentImport.test.ts` covers derived export columns (`grand_total_marks`, per-rubric `:marks`, question columns) reported as explicit Ignored Columns, never imported and never blocking. Design locked 2026-06-22 (`plans/active/2026-06-22-r009-import-export-roundtrip-contract.md`, grilled via `/grill-with-docs`): a marks-only file (zero header columns matching a project rubric) becomes a new `no-assessment-columns` Blocking Diagnostic rather than a silent zero-write success; roundtrip fidelity gets a new `src/import/assessmentImportRoundtrip.integration.test.ts`. | Implement `plans/active/2026-06-22-r009-import-export-roundtrip-contract.md`: add the `no-assessment-columns` blocking diagnostic, the roundtrip integration test (full export, assessment-only export, marks-only export), then promote to Verified. |
| R-010 | Tier 1 | 24 | Numerical rubric math | markNumericalRubric boundary behavior (score range edges, reversed mode) needs broader coverage beyond current happy-path tests. | src/rubrics/rubric.ts, src/rubrics/rubric.test.ts | Open | [#23](https://github.com/QuentinRoy/grading/issues/23) | Unassigned | Sprint C | Pending | Add characterization tests for score interpolation and reversed mapping at min/max edges; assert the existing throws for out-of-range scores and inverted min/max. `markNumericalRubric` already guards these, so no behavior change is expected. |
