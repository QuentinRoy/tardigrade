...
- Tier 2: 2 open, 0 in progress, 2 verified
...
- Remaining scope: R-013, R-014 (Tier 2)
...
| R-015 | Tier 2 | 20 | Operational signal quality | Some paths surface generic errors; consistency of user-recoverable guidance should be tested and standardized. | app/projects/page.tsx, src/import/actionUtils.ts | Verified | [#29](https://github.com/QuentinRoy/grading/issues/29) | Unassigned | Sprint C | Implemented per `plans/active/2026-06-23-tier2-action-route-ux-hardening.md` PR 1, alongside R-012 (same error-classification rule). `app/projects/page.tsx`'s error-shaping logic moved into a new co-located `app/projects/createProjectErrorMessage.ts` (kept out of the JSX page module so it unit-tests without pulling in MUI): `toCreateProjectErrorMessage` keeps the specific "Project name is required." message and otherwise logs under the `projects` scope and returns a generic `projectMessages.unexpected` message — never the raw `error.message`. `src/questions/errors.ts`'s `toQuestionsValidationError` keeps `QuestionsValidationError`/`ZodError` branches and now logs under `questions` + returns a generic `questionsMessages.unexpected` message for any other error. Audited the bare-`Error` trap at `questionDefinitionMutations.ts:85` ("Question id is required.", defensive/unreachable given `idSchema`'s zod `min(1)`) and routed it through `QuestionsValidationError` so it stays a recognized domain message. `src/import/actionUtils.ts`'s `toImportErrorState` gained the same generic-fallback + single-log-call shape via the new `ImportBlockedError` type (see R-012). `app/projects/createProjectErrorMessage.test.ts` and `src/questions/errors.test.ts` assert the specific message is kept, the generic message never leaks internal text, and the scoped logger is called exactly once for unexpected errors. The unit Vitest project's `include` glob (`vitest.config.ts`) was extended to also match `app/**/*.{test,spec}.{ts,tsx}` so the new co-located `app/projects/createProjectErrorMessage.test.ts` is discovered (previously only `src/**` was globbed; pulled forward from PR 3's planned scope because it was needed to make this PR's own test enforceable in CI). local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm run test:unit` (177 passing). | None — keep evidence current if these modules move. |
