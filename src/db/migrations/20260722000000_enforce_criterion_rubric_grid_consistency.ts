...
type MigrationDB = {
	criterion: { id: string; rubric_id: number; grid_row_id: number };
	rubric: { row_id: number; grid_row_id: number };
};
...
export async function up(db: Kysely<MigrationDB>): Promise<void> {
	...
	const violations = await db
		.selectFrom("criterion as c")
		.innerJoin("rubric as r", "r.row_id", "c.rubric_id")
		.whereRef("c.grid_row_id", "<>", "r.grid_row_id")
		.select("c.id")
		.execute();

	if (violations.length > 0) {
		const offendingCriterionIds = violations.map((row) => row.id).join(", ");
		throw new Error(
			`Cannot enforce criterion/rubric grid consistency: ${violations.length} ` +
