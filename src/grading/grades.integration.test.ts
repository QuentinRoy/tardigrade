...
// and declares `grids:{gridId}:grades:all` alongside its granular tag: bulk imports only bust
...
test("loadRubricGrade wrapper delegates to its primitive and declares its cache tags", async () => {
	...
	expect(declaredTags).toContain(`grids:${fixture.gridId}:grades:all`);
...
// `grids:{gridId}:grades:all` (busted by bulk imports) or the overview would serve stale data.
