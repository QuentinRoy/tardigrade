...
const createCsvGradeTargetExport = vi.fn();
...
vi.mock("#export/gradeTargetExport.ts", () => ({
	createCsvGradeTargetExport: (...args: unknown[]) =>
		createCsvGradeTargetExport(...args),
}));
...
function buildRequest(query = ""): Request {
	return new Request(
		`https://example.test/projects/project-1/intro-to-grading/export/grades${query}`,
	);
}
...
describe("GET /export/grades", () => {
	...
	it("returns 404 with an error body when the project is not found", async () => {
		...
		expect(createCsvGradeTargetExport).not.toHaveBeenCalled();
	});
	...
	it("returns a 200 CSV stream with dated filename, content-type, and no-store cache-control", async () => {
		...
		createCsvGradeTargetExport.mockResolvedValue(csvStream());
	...
	it("returns 500 with an error body and logs when the export throws", async () => {
		...
		createCsvGradeTargetExport.mockRejectedValue(new Error("db exploded"));
