...
- `.agents/skills/error-handling-ux/SKILL.md` governs *user-facing* error messages; this ADR governs *operational* logging that is never shown to a user. The two are not in tension, but a future contributor should not conflate "log it" with "tell the user."
