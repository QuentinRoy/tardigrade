...
- **Mantine defaults first.** Use Mantine's built-in color-scheme support and component defaults without app-level density or visual-token tuning for now. Introduce app-owned theme tokens only when a concrete, cross-cutting need emerges.
