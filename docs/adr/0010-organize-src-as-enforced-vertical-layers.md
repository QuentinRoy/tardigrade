...
- Open execution decisions are tracked in [the source-structure investigation](../investigations/2026-06-28-source-structure-product-verticals.md) and a forthcoming `plans/` plan: the file-by-file split of `assessments`, whether `imports` gains per-flow subverticals, the placement of quick-jump and the cross-cutting `ui` pieces, vertical carve order, and an optional later `AssessmentRubricValue` rename.
