...
- **infrastructure (leaf)** (`src/{db,utils}`): `db` per ADR 0002; runtime helpers.
- **test support** (`src/test`): shared test tooling — global setup, DB fixture primitives, IDs (`docs/reference/testing-conventions.md`). Not part of the leaf: it composes verticals and shared-domain to build fixtures (e.g. a rubric fixture calling into `rubric-management`), so it cannot be downstream-only like `db`/`utils`. Instead it gets the opposite boundary — production code must never import `src/test`; only test files may. Exempting the whole directory from rule 1 the way test files are exempted would let it import every vertical *and* let verticals import it back, turning it into a hidden cross-vertical coupling hub.
...
8. `src/test` is a top-level test-support layer, not part of the infrastructure leaf: production code (anything outside a test file) must never import `src/test`. Enforced by dependency-cruiser.
...
- 2026-07-21: Split `src/test` out of the infrastructure-leaf bucket into its own "test support" layer: it composes verticals/shared-domain to build fixtures, so it cannot be downstream-only like `db`/`utils`. Added Rule 8 and a matching dependency-cruiser rule enforcing that production code must never import `src/test` (only test files may), rather than exempting the whole directory from vertical isolation the way test files are exempted.
