...
- **Created:** 2026-05-20
- **Related:** #99
- **Note (2026-07-06, updated 2026-07-15):** The *terminology* is resolved (see `CONTEXT.md`): the value pipeline is **Grade → Mark → Total** for every criterion kind (Check, Options, Number — the old boolean/ordinal/numerical kind ids are retired), `Grade` names the atomic per-criterion record and the act of grading, `Mark` is the numeric worth a grade is computed to, and `Total` is the aggregate of marks. `Score` is fully retired in favor of **Value**: a Number grade's payload is its Value (a Check grade has a Yes/No answer, an Options grade has a label — neither has a Value), and Value is a payload name, not a pipeline stage. `Points` stays avoided. The *structural* questions remain open and keep this Active: whether aggregation is a plain sum or `Σ mark × weight`, where weighting lives, and whether normalization is needed. Total is implemented (`src/results/resultsBuilder.ts` sums marks per Grade Target; the Grades table's Total column; the `final_total` export column) — the open question is the weighting formula, not whether Total exists.
